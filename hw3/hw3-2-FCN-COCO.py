...
Used by Weiheng Li
My laptop is too slow, I can't wait the whole program to the finish. But I believe my code
is all good if running on a faster machine.
To run it, you should put vgg16.npy in the root (500MB+), as shown below. And keep the other files 
...
from fcnn import FCNN, bn_relu_conv
...
# vgg16.npy download: https://mega.nz/#!YU1FWJrA!O1ywiCS2IiOlUCtCpI6HTJOMrneN-Qdv3ywQP5poecM
import vgg16
...
if __name__ == "__main__":
    ...
    for im_id in range(32):
        plt.subplot2grid((8, 8), (2 * int(im_id / 8), im_id % 8))
        plt.axis('off')
        plt.imshow(ims_ts[im_id], interpolation='nearest', vmin=0, vmax=1)

        plt.subplot2grid((8, 8), (2 * int(im_id / 8) + 1, im_id % 8))
        plt.axis('off')
        plt.imshow(ims_ts[im_id], interpolation='nearest', vmin=0, vmax=1)
